...
```{r}    
# for loop
output <- vector("character", length = length(month.name))
for (i in seq_along(month.name)) {
  output[i] <- str_c("Month ", i, ": ", month.name[i])
}
...
diamonds |> 
    mutate(across(.cols = c(carat, x, y, z), .fns = \(x) round(x, digits = 1) ))
...
diamonds |>
    group_by(cut) |> 
    summarize(across(.cols = where(is.numeric), .fns = list(mean = \(x) mean(x, na.rm = T), med = \(x) median(x, na.rm = T), sd = \(x) sd(x, na.rm = T))))
```
...
```{r}
diamonds %>%
  mutate(across(.cols = c(x, y, z), .fns = \(x) str_c(x, " mm")))
```
