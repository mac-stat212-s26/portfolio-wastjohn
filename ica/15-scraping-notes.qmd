...
```{r}
# I'm gonna web scrape the MIfA website for their list of grad programs
scrape_MIfA_page <- function(url){
    Sys.sleep(2)
    page <- read_html(url)
    class_titles <- get_text_from_page(page, "h3")
    paragraphs <- get_text_from_page(page, "p")
    
    tibble(
        title = class_titles[1:length(class_titles)-1],
        offered = paragraphs[str_starts(paragraphs, "Typically Offered")],
        credits = paragraphs[str_starts(paragraphs, "\\d")],
        prereqr = paragraphs[str_starts(paragraphs, "Prerequisite")],
        description = paragraphs[str_starts(paragraphs, "Prerequisite|Typically Offered|\\d", negate=TRUE)]
    ) |> 
    separate_wider_delim(
    delim = ": ", 
    cols = date_abstract, 
...
url <- "https://cse.umn.edu/mifa/grad/courses"
```
