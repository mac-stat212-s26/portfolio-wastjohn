...
```{r ex_create_string}
# Your code
mystr <- "It's Thursday"  # note: single quotes gives an error because of the odd number of quotes
str_view(mystr)
```
...
# Your code
mystr <- "C:\\Users"  # if you don't escape, the computer thinks it's hexcode
str_view(mystr)
```
...
df_dates %>%
  mutate(date1 = str_c(month, "-", day, "-", year), date2 = str_glue("{month}-{day}-{year}"))
...
df %>%
  mutate(middle_letter = str_sub(word, start = ceiling(str_length(word) / 2), end = ceiling(str_length(word) / 2)))
...
#| eval: false
str_view(c("a", "ab", "abb"), "ab?")  # contains a and (b either once or not at all) --> a, ab, abb
str_view(c("a", "ab", "abb"), "ab+")  # ab, abb
str_view(c("a", "ab", "abb"), "ab*")  # a, ab, abb
```
