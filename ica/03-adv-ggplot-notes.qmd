...
> **Response**: The NYTimes is repeating the claim that 2015 was the hottest year on record, giving readers the opportunity to explore if their personal city agrees with the statment. This goal caused them to make a text box to enter their city, display their city's 2015 temperature data, and display historical data for comparison.
...
> **Response**: The overall plot is temperature vs time, with months labeled and temperature containing actual high and low information for 2015 as well as presumably the average in the past. The historic data is greyed out and behind the 2015 data, which is colored a bright red/pink.
...
> **Response**: You could use `geom_poly()` to make rectangles, or `geom_segment()` and increase the size of the brush.
...
```{r }
month.boundaries <- weather %>% group_by(Month) %>% filter(Day == max(Day)) %>% ungroup() %>% select(dateInYear) %>% head(11)
...
ggplot(weather) +
  ...
  geom_vline(
    data = month.boundaries,
    aes(
      xintercept = dateInYear
      ),
    linetype = "dashed",
    alpha = 0.25
    ) +
...
```{r fig.width=10}
month.boundaries <- weather %>% group_by(Month) %>% filter(Day == max(Day)) %>% ungroup() %>% select(dateInYear) %>% head(11)
month.labels <- weather %>% group_by(Month) %>% summarize(start = min(dateInYear), end = max(dateInYear), mid = (start + end) / 2)
...
cp_rec <- weather %>%
  group_by(Month) %>%
  summarize(cp = max(CulmPrec)) %>%
  ungroup() %>%
  select(cp)

cp_rec <- cbind(cp_rec, weather %>% group_by(Month) %>% filter(Day == max(Day)) %>% ungroup() %>% select(dateInYear))
