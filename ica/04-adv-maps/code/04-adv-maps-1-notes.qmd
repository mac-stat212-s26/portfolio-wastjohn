...
transform_and_plot(nc, new_crs = "EPSG:32133", title="North Carolina")
...
> **Response**: We would expect an overlay of the same point in both projections to result in two separate points that don't overlap well, especially near the edges. 
...
st_crs(mn_counties)  # WGS84 --> need to change

mn_counties <- mn_counties %>% st_transform(crs = st_crs(mn_cities))
st_crs(mn_counties)  # all better now!
```
...
> **Response**: Viridis is a colorblind-friendly colormap. `_d()`, `_c()`, and `_b()` correspond to a discrete, continuous, and binned
...
elevation <- elevatr::get_elev_raster(mn_counties, z = 5, clip = "bbox")  # get elevation values for a given CRS
...
png("../figures/tc_map_zoom.png", width = 200, height = 200)
