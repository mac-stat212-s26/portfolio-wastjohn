...
> a. MCAR
> b. MCAR unless they are missing because of extreme temperatures
> c. MNAR
...
> The missing values are not evenly distributed along the Solar.R values in either case. Additionally, Mod1 has existing values where the missing values are, so we could extrapolate, but that is not the case for Mod2.
...
# month
airquality %>%
  ggplot(aes(y = Month, x = is.na(Ozone))) + 
  geom_boxplot()  

airquality %>%
  ggplot(aes(x = Month, color = is.na(Ozone))) + 
  geom_density() # most of the missing points in ozone are in month 6

# wind
airquality %>%
  ggplot(aes(y = Wind, x = is.na(Ozone))) + 
  geom_boxplot()  

airquality %>%
  ggplot(aes(x = Wind, color = is.na(Ozone))) + 
  geom_density() # missing and non-missing are very similar

# temp
airquality %>%
  ggplot(aes(y = Temp, x = is.na(Ozone))) + 
  geom_boxplot()  
...
airquality %>%
  ...
  geom_smooth()
```
> I think we could use month as a predictor for imputating the missing values
