...
phone.number <- function(phone){
  ...
  phone <- str_replace_all(phone, " ", "")
  area <- str_sub(phone, 1, 3)
  lead <- str_sub(phone, 4, 6)
  four <- str_sub(phone, 7, 10)
  return(str_c("(", area, ")", " ", lead, "-", four))
}

phone.number(c("651-330-8661", "6516966000", "800 867 5309"))
...
> **Answer**: The arguments in geom_sf() are ordered mapping, then data.
...
```{r}
get.domain <- function(email){
  position <- str_locate(email, "@")
  return(str_sub(email, position[1]+1, str_length(email)))
}

get.domain("wastjohn@macalester.edu")
get.domain("foo@bar.com")
```
...
```{r}
count <- function(df, group_var) {
  df %>%
    group_by(pick({{ group_var }})) %>%
    summarize(count = n(), prop = count / nrow(df))
}
...
scat.smooth(diamonds, carat, price)
```
## Done!
